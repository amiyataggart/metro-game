...
- **Declarative cartography overrides** (`ORDER_OVERRIDES` / `NO_SNAP` in `build-ribbons.js`, re-applied on every build → survive a data re-fetch) → fixed: Mile End (Central/H&C/District top→bottom), Jubilee above Metropolitan, Suffragette below Victoria, Mildmay passes over West Hampstead unaffected. Stronger laneOff smoothing (`OFFSET_SMOOTH_WIN`) reduced the Thameslink wobble at Finsbury Park.
